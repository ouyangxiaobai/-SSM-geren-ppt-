<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="出诊时间" prop="chuzhenshijian">
          <el-input v-model="ruleForm.chuzhenshijian" 
              placeholder="出诊时间" clearable  :readonly="ro.chuzhenshijian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="出诊时间" prop="chuzhenshijian">
              <el-input v-model="ruleForm.chuzhenshijian" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="态度评分" prop="taidupingfen">
          <el-select v-model="ruleForm.taidupingfen" placeholder="请选择态度评分">
            <el-option
                v-for="(item,index) in taidupingfenOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="态度评分" prop="taidupingfen">
	      <el-input v-model="ruleForm.taidupingfen"
                placeholder="态度评分" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shijianduan = true;
          ...
          if(o=='taidupingfen'){
            this.ruleForm.taidupingfen = obj[o];
	    this.ro.taidupingfen = true;
            continue;
          }
          if(o=='jishupingfen'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiuzhenpingjia/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiuzhenpingjiaCrossAddOrUpdateFlag = false;
                  this.parent.search();
