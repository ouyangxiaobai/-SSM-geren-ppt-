  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
	    ...
	    <download-excel v-if="isAuth('yuyueguahao','导出')" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "预约挂号.xls">
		      <!-- 导出excel -->
            	<el-button
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('yuyueguahao','评价') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="jiuzhenpingjiaCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'评价':'' }}</el-button>
                <el-button v-if="isAuth('yuyueguahao','评价') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="jiuzhenpingjiaCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'评价':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yuyueguahao','评价') && contents.tableBtnIcon == 0" type="success" size="mini" @click="jiuzhenpingjiaCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'评价':'' }}</el-button>
    ...
    <jiuzhenpingjia-cross-add-or-update v-if="jiuzhenpingjiaCrossAddOrUpdateFlag" :parent="this" ref="jiuzhenpingjiaCrossaddOrUpdate"></jiuzhenpingjia-cross-add-or-update>
...
import jiuzhenpingjiaCrossAddOrUpdate from "../jiuzhenpingjia/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      jiuzhenpingjiaCrossAddOrUpdateFlag: false,
...
//导出excel
      json_fields: {
      "医生工号": "yishenggonghao",    //常规字段
      "医生姓名": "yishengxingming",    //常规字段
      "职称": "zhicheng",    //常规字段
      "科室": "keshi",    //常规字段
      "挂号费": "guahaofei",    //常规字段
      "出诊时间": "chuzhenshijian",    //常规字段
      "时间段": "shijianduan",    //常规字段
      "就诊描述": "jiuzhenmiaoshu",    //常规字段
      "日期": "riqi",    //常规字段
      "用户名": "yonghuming",    //常规字段
      "姓名": "xingming",    //常规字段
  ...
  components: {
    AddOrUpdate,
    jiuzhenpingjiaCrossAddOrUpdate,
  },
  methods: {
    contentStyleChange() {
    ...
    jiuzhenpingjiaCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.jiuzhenpingjiaCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','yuyueguahao');
      this.$nextTick(() => {
      this.$refs.jiuzhenpingjiaCrossaddOrUpdate.init(row.id,type);
      });
    },
    payHandler(row){
